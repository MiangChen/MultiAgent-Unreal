...
AMAChargingStation::AMAChargingStation() {
  ...
  MeshComponent->SetCollisionResponseToChannel(ECC_Visibility,
                                               ECR_Block); // 允许射线检测
  MeshComponent->SetCollisionResponseToChannel(ECC_Camera,
                                               ECR_Block); // 允许相机碰撞;

  // Set default soft references (can be changed in editor)
  StationMeshAsset = TSoftObjectPtr<UStaticMesh>(FSoftObjectPath(TEXT(
      "/Game/Props/ChargingStation/SM_StationRecharge.SM_StationRecharge")));
  StationMaterialAsset = TSoftObjectPtr<UMaterialInterface>(FSoftObjectPath(
      TEXT("/Game/Props/ChargingStation/M_ChargingStation.M_ChargingStation")));

  // 在构造函数中加载默认 Mesh（编辑器中可见）
  static ConstructorHelpers::FObjectFinder<UStaticMesh> DefaultMesh(TEXT(
      "/Game/Props/ChargingStation/SM_StationRecharge.SM_StationRecharge"));
...
void AMAChargingStation::BeginPlay() {
  ...
  InteractionSphere->OnComponentBeginOverlap.AddDynamic(
      this, &AMAChargingStation::OnOverlapBegin);
  InteractionSphere->OnComponentEndOverlap.AddDynamic(
      this, &AMAChargingStation::OnOverlapEnd);
}

void AMAChargingStation::OnOverlapBegin(UPrimitiveComponent *OverlappedComp,
                                        AActor *OtherActor,
  ...
  if (AMACharacter *Robot = Cast<AMACharacter>(OtherActor)) {
    ...
    // 充电由 Charge 技能处理
    UE_LOG(LogTemp, Log, TEXT("[ChargingStation] %s entered range"),
           *Robot->AgentName);
  }
}

void AMAChargingStation::OnOverlapEnd(UPrimitiveComponent *OverlappedComp,
                                      AActor *OtherActor,
                                      UPrimitiveComponent *OtherComp,
  ...
  if (AMACharacter *Robot = Cast<AMACharacter>(OtherActor)) {
    ...
    UE_LOG(LogTemp, Log, TEXT("[ChargingStation] %s left range"),
           *Robot->AgentName);
  }
}
