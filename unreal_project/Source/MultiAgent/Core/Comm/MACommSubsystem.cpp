...
{
    UE_LOG(LogMACommSubsystem, Log, TEXT("UE5 -> Python: TimeStep %d feedback (%d agents)"), 
...
void UMACommSubsystem::SendMessageEnvelope(const FMAMessageEnvelope& Envelope)
{
    ...
    UE_LOG(LogMACommSubsystem, Verbose, TEXT("SendMessageEnvelope: Type=%d, MessageId=%s"), 
...
void UMACommSubsystem::OnPollRequestComplete(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bConnectedSuccessfully)
{
    ...
    if (ResponseCode == 200)
    {
        ...
        {
            UE_LOG(LogMACommSubsystem, Log, TEXT("Python -> UE5: Received data"));
...
void UMACommSubsystem::HandlePollResponse(const FString& ResponseJson)
{
    ...
    for (const TSharedPtr<FJsonValue>& MsgValue : MessageArray)
    {
        ...
        UE_LOG(LogMACommSubsystem, Log, TEXT("Python -> UE5: Message type: %s"), *MessageTypeStr);
        ...
        else if (MessageTypeStr == TEXT("skill_list"))
        {
            ...
            if (MsgObject->TryGetObjectField(TEXT("payload"), PayloadObject))
            {
                ...
                {
                    UE_LOG(LogMACommSubsystem, Log, TEXT("HandlePollResponse: Received SkillList with %d time steps"),
                ...
                {
                    UE_LOG(LogMACommSubsystem, Warning, TEXT("HandlePollResponse: Failed to parse SkillList payload"));
                }
            }
        }
        else if (MessageTypeStr == TEXT("query_request"))
        {
            ...
            if (MsgObject->TryGetObjectField(TEXT("payload"), PayloadObject))
            {
                ...
                UE_LOG(LogMACommSubsystem, Log, TEXT("HandlePollResponse: Received query request: %s"), *QueryType);
                
                HandleQueryRequest(QueryType, *PayloadObject);
            }
        }
        else
        {
...
{
    UE_LOG(LogMACommSubsystem, Log, TEXT("SendWorldStateResponse: QueryType=%s"), *QueryType);
