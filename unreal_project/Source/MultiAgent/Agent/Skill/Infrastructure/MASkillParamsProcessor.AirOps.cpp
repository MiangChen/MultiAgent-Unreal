...
namespace
{
FMALidarProbeConfig BuildLandingGroundProbeConfig()
{
    FMALidarProbeConfig Config;
    Config.MaxDistance = 30000.f;
...
void FMASkillParamsProcessor::ProcessLand(UMASkillComponent* SkillComp, const FMAAgentSkillCommand* Cmd)
{
    ...
    else
    {
        const FVector ProbeOrigin(
            LandLocation.X,
            LandLocation.Y,
            FMath::Max(AgentLocation.Z + 3000.f, 12000.f));
        const FMALidarProbeResult ProbeResult =
            FMALidarProbe::SampleCone(*World, ProbeOrigin, FVector::DownVector, BuildLandingGroundProbeConfig(), Agent);

        if (ProbeResult.bHasHit)
        {
            LandHeight = ProbeResult.MeanImpactPoint.Z;
        }
    }
