...
void USK_Search::GenerateSearchPath()
{
    ...
    const int32 PreviewCount = FMath::Min(SearchPath.Num(), 4);
    for (int32 Index = 0; Index < PreviewCount; ++Index)
    {
        const FVector& Waypoint = SearchPath[Index];
        UE_LOG(LogTemp, Log, TEXT("[SK_SearchDebug] %s: Waypoint[%d/%d] = (%.0f, %.0f, %.0f)"),
            *Character->AgentLabel,
            Index + 1, SearchPath.Num(),
...
void USK_Search::NavigateToNextWaypoint()
{
    ...
    const FVector TargetLocation = SearchPath[CurrentWaypointIndex];

    UE_LOG(LogTemp, Log, TEXT("[SK_SearchDebug] %s: Navigating to waypoint %d/%d at (%.0f, %.0f, %.0f) from (%.0f, %.0f, %.0f)"),
    ...
    if (NavigationService->bIsFlying)
    {
        if (UWorld* World = Character->GetWorld())
        {
            ...
            if (Now - LastStuckDebugLogTime >= 3.0f)
            {
                LastStuckDebugLogTime = Now;
                const FVector CurrentLocation = Character->GetActorLocation();
                UE_LOG(LogTemp, Log, TEXT("[SK_SearchDebug] %s: Waypoint %d/%d tracking started, dist2D=%.1f, dz=%.1f"),
                    *Character->AgentLabel,
                    CurrentWaypointIndex + 1, SearchPath.Num(),
                    FVector::Dist2D(CurrentLocation, TargetLocation),
