...
bool UMANavigationService::EnsureCanRunFlightCommand(const TCHAR* CommandName)
{
    if (!OwnerCharacter)
    {
        UE_LOG(LogTemp, Warning, TEXT("[MANavigationService] %s failed: No owner character"), CommandName);
        return false;
    }

    if (!bIsFlying)
    {
        UE_LOG(LogTemp, Warning, TEXT("[MANavigationService] %s: %s failed: Not a flying vehicle"),
            *OwnerCharacter->GetName(), CommandName);
        return false;
    }
    ...
    {
        UE_LOG(LogTemp, Warning, TEXT("[MANavigationService] %s: %s failed: FlightController not initialized"),
            *OwnerCharacter->GetName(), CommandName);
        return false;
    }
