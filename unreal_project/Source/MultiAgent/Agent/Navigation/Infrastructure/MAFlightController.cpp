...
namespace
{
FMALidarProbeConfig BuildDownwardGroundProbeConfig()
{
    FMALidarProbeConfig Config;
    Config.MaxDistance = 70000.f;
    Config.HalfAngleDegrees = 6.f;
    Config.RingCount = 2;
...
void FMAMultiRotorFlightController::UpdateFlight(float DeltaTime)
{
    ...
    const float HorizontalDistance = FVector::Dist2D(CurrentLocation, TargetLocation);
    ...
    if (State == EMAFlightControlState::Landing)
    {
        ...
        const float LandingHorizontalDistance = HorizontalOffset.Size();
        const float LandingVerticalDistance = FMath::Abs(ToTarget.Z);
        ...
        if (LandingHorizontalDistance <= HorizontalCompleteThreshold && LandingVerticalDistance <= VerticalCompleteThreshold)
        {
            CurrentSpeed = 0.f;
            SetState(EMAFlightControlState::Idle);
            return;
        ...
        if (LandingHorizontalDistance > KINDA_SMALL_NUMBER)
        {
            const float HorizontalStep = FMath::Min(LandingHorizontalDistance, HorizontalLandingSpeed * DeltaTime);
        ...
        CurrentSpeed = LandingHorizontalDistance > KINDA_SMALL_NUMBER ? HorizontalLandingSpeed : 0.f;
    ...
    if (Owner && Owner->GetWorld())
    {
        ...
        const bool bNearTarget = HorizontalDistance <= FMath::Max(500.f, AcceptanceRadius * 4.f);
        const float AvoidanceAlignment = FVector::DotProduct(Direction, AvoidanceDirection);
        const bool bHeavyAvoidance = AvoidanceAlignment < 0.85f;
        if ((bNearTarget || bHeavyAvoidance) && (Now - LastDebugLogTime >= 1.0f))
        {
            LastDebugLogTime = Now;
            UE_LOG(LogTemp, Log,
                TEXT("[FlightDebug] %s: state=%d target=(%.0f, %.0f, %.0f) current=(%.0f, %.0f, %.0f) dist3D=%.1f dist2D=%.1f dz=%.1f speed=%.1f acceptance=%.1f avoidAlign=%.2f"),
                *Owner->GetName(),
                static_cast<int32>(State),
                TargetLocation.X, TargetLocation.Y, TargetLocation.Z,
                CurrentLocation.X, CurrentLocation.Y, CurrentLocation.Z,
                Distance, HorizontalDistance, VerticalDistance, CurrentSpeed, AcceptanceRadius, AvoidanceAlignment);
        }
    }
...
{
    if (!Owner || !Owner->GetWorld()) return 0.f;

    const FVector SampleLocation = Owner->GetActorLocation();
    const FVector ProbeOrigin(SampleLocation.X, SampleLocation.Y, FMath::Max(SampleLocation.Z + 5000.f, 50000.f));
    const FMALidarProbeResult ProbeResult =
        FMALidarProbe::SampleCone(*Owner->GetWorld(), ProbeOrigin, FVector::DownVector, BuildDownwardGroundProbeConfig(), Owner);
    return ProbeResult.bHasHit ? ProbeResult.MeanImpactPoint.Z : 0.f;
}
