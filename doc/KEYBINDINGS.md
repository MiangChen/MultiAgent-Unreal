...
Edit 模式用于模拟任务执行过程中发生的"新情况"（动态变化）。与 Modify 模式（持久化修改源场景图文件）不同，Edit 模式的所有操作仅针对临时场景图文件进行，不影响源文件。进入该模式后，屏幕右上角显示蓝色 "Mode: Edit (M)" 指示器，屏幕右侧显示编辑面板。
