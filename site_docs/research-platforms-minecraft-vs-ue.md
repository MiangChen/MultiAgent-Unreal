...
相比表格，这里用“集合”的方式表达更直观：

- 左侧是 `UE` 更自然的任务。
- 中间是 `UE ∩ Minecraft` 都能做的任务。
- 右侧是 `Minecraft` 更自然的任务。
- `置信度` 表示“这个任务归类到该集合”的把握，不是任务成功率。

```mermaid
flowchart LR
    UE((UE 专属任务集))
    BOTH((交集<br/>UE ∩ Minecraft))
    MC((Minecraft 专属任务集))

    UE --- BOTH
    BOTH --- MC

    classDef ue fill:#dceafb,stroke:#6f97bf,stroke-width:2px,color:#1c2733;
    classDef both fill:#e7f7e9,stroke:#7eae84,stroke-width:2px,color:#1e2a21;
    classDef mc fill:#fff0cc,stroke:#c7a25a,stroke-width:2px,color:#2f2618;
...
`U1` 先派 `UAV` 建图，再让 `UGV` 进入局部巡检，最后由 `humanoid` 执行近距离确认。置信度 `90%`。

`U2` 无人机在厂区、管廊或园区中做低空视觉巡检，同时考虑禁飞区、相机姿态稳定和碰撞余量。置信度 `95%`。

`U3` `UAV + UGV + humanoid` 对同一目标做跨视角接力跟踪，处理遮挡、丢失和重识别。置信度 `90%`。

`U4` `UGV` 依据 `UAV` 的先验地图进入建筑内部，穿越坡道、楼梯、门洞并完成近距检查。置信度 `88%`。

`U5` 无人机发现异常后，引导地面机器人到达指定点并完成 rendezvous / handoff。置信度 `85%`。

`U6` 异构集群在带车辆、行人、动态障碍的场景中执行巡逻、避障与重规划。置信度 `92%`。

`U7` 使用 `RGB + Depth + LiDAR` 做融合建图，再用于探索、定位和回环。置信度 `93%`。

`U8` 机械臂抓取盒子并放到车上，或从货架取物后交给移动机器人。置信度 `98%`。

`U9` 两台机器人协同搬运长杆、天线等细长物体，穿过狭窄门洞并保持姿态稳定。置信度 `98%`。

`U10` 基于真实几何、质量、碰撞和约束完成 pick-up、place、carry、dock、charge。置信度 `97%`。

### 5.2 交集任务集

`C1` 视觉导航到指定目标点。置信度 `95%`。

`C2` 未知区域探索并提高地图覆盖率。置信度 `90%`。

`C3` 搜索指定目标物或目标体。置信度 `90%`。

`C4` 单目标主动视觉跟踪。置信度 `75%`。

`C5` 多 agent 协同搜索并在指定地点会合。置信度 `80%`。

`C6` 语言指令驱动的多步任务执行。置信度 `88%`。

`C7` 中央决策器基于结构化状态做任务分配与子目标下发。置信度 `90%`。

`C8` 把资源、样本或简单任务载荷从 `A` 点送到 `B` 点。置信度 `78%`。

`C9` 巡逻、重访关键区域、按路线依次检查。置信度 `85%`。

`C10` 简单环境改造以满足任务目标。置信度 `70%`。

### 5.3 Minecraft 专属任务集

`M1` 从零开始采木头、做工作台、做镐、挖铁、熔炼，最后采到钻石。置信度 `95%`。

`M2` 在不能“往地里直挖”的约束下，探索并找到洞穴。置信度 `95%`。

`M3` 在村庄旁建一座风格一致的村屋，并按要求展示成果。置信度 `92%`。

`M4` 在山地中造一个瀑布，并移动视角拍出合格画面。置信度 `92%`。

`M5` 在村庄附近围一个动物栏，抓两只同类动物进去，且尽量不破坏村庄结构。置信度 `95%`。

`M6` 围绕 `wood -> stone -> iron -> diamond` 这类 tech-tree 做长链条 crafting / planning。置信度 `94%`。

`M7` 以“发现尽可能多新物品或技能”为目标做 open-ended exploration。置信度 `90%`。

`M8` 围绕 inventory、recipe、smelting 做长时程资源规划。置信度 `93%`。

`M9` 根据多模态提示完成 build、break、farm、smelt 这类 TeamCraft 风格任务。置信度 `88%`。

`M10` 在 MineLand 一类环境里做“有限感知 + 生存需求 + 多 agent 协作”的生态任务。置信度 `85%`。
