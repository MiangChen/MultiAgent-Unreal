...
| 研究任务 | 更适合的平台 | 原因 |
|---|---|---|
| 长时程 sparse reward RL | Minecraft | 从树木、工具、熔炉到钻石，天然就是分层技能链 |
| 模仿学习 / human priors | Minecraft | MineRL 长期围绕人类演示数据组织 benchmark |
| 人类反馈 / 偏好学习 | Minecraft | BASALT 就是围绕“难以写 reward 的任务”设计 |
| 语言条件下的开放任务 | Minecraft | MineDojo 提供任务文本、互联网知识和大规模任务库 |
| 开放世界探索与生存 | Minecraft | 程序生成世界和 tech-tree 结构天然合适 |
| 视觉合成数据 | UE | UnrealCV、原生 UE 都更适合高保真数据生产 |
| 自动驾驶闭环评测 | UE | CARLA 的评测协议、场景和指标更成熟 |
| UAV 巡检 / 飞控 / 视觉导航 | UE | AirSim 和原生 UE 在传感器与控制上更自然 |
| 多机器人异构协同 | UE | UE 更适合表达机器人、载具、通信与场景对象 |
| 刚体搬运 / 抓取 / 装配 | UE | 需要真实碰撞、质量、约束、接触稳定性 |
| 数字孪生 / HITL | UE | 更容易对接真实地图、资产、交互 UI 和操作流程 |
| 低成本 benchmark 复现 | Minecraft | 现成 benchmark 清晰，任务复现成本较低 |
