...
本页保留当前仓库的一张面向使用者 workflow 图、两张基线图，以及一个关键子图：
- 面向使用者 workflow 图
- 自动控制架构图
- folder 映射图
...
说明：
- 这张图从使用者视角描述项目的主流程：目标设定、外部算法规划、backend 下发、HITL 审核、UE 执行，以及反馈回流。
- `HITL` 是中心门控节点，决定候选命令能否真正进入执行层。
- `world_state`、`scene_change` 与 `message` 共同构成观测层，支撑使用者调整与外部算法重新规划。
- 这张图用于说明“怎么使用系统”，不是源码 folder 落位图。

```mermaid
%%{init: {"theme":"base","flowchart":{"htmlLabels":true,"curve":"basis","nodeSpacing":40,"rankSpacing":55}}}%%
flowchart TB
    subgraph L1["委托层"]
        direction TB
        U["使用者<br/>设定任务目标<br/>约束条件与偏好"]
        UA["使用者调整<br/>根据反馈修正<br/>策略与约束"]
    end

    subgraph L2["代理层"]
        direction TB
        P["外部算法<br/>读取 world_state<br/>生成执行方案"]
        B["Mock Backend<br/>封装协议消息<br/>发送候选命令"]
    end

    subgraph L3["监督层"]
        direction TB
        H["HITL 审核中心<br/>批准执行 / 驳回修改<br/>人工保持控制权"]
    end

    subgraph L4["执行层"]
        direction TB
        E["UE 执行系统<br/>解析任务与技能<br/>调度机器人动作"]
        R["机器人与场景<br/>移动 感知 交互<br/>产生状态变化"]
    end

    subgraph L5["观测层"]
        direction TB
        F["反馈层<br/>message / scene_change<br/>world_state 回流"]
    end
    ...
    style L1 fill:#FFF4C9,stroke:#D9C46B,stroke-width:1.4px
    style L2 fill:#DFF3E4,stroke:#96C7A1,stroke-width:1.4px
    style L3 fill:#F8DDE1,stroke:#D99AA4,stroke-width:1.4px
    style L4 fill:#DCEBFA,stroke:#97B7D7,stroke-width:1.4px
    style L5 fill:#E9E1F8,stroke:#B39FD6,stroke-width:1.4px

    classDef userNode fill:#FFFDF5,stroke:#D4BE67,color:#333,stroke-width:1.2px
    classDef agentNode fill:#F6FFF8,stroke:#89B996,color:#333,stroke-width:1.2px
    classDef reviewNode fill:#FFF7F8,stroke:#CE8894,color:#333,stroke-width:1.4px
    classDef execNode fill:#F7FBFF,stroke:#88A9CC,color:#333,stroke-width:1.2px
    classDef obsNode fill:#FBF9FF,stroke:#A792CF,color:#333,stroke-width:1.2px
```
